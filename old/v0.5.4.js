...
function installFunctions() {
  if (!(installed||fatal_error)) {
    ...
    let menu = ui.createMenu('Google Docs Writing Engine v0.5.3').addItem('✍🏻字數統計(Word Count)', 'countPerSection');
    let subMenu = ui.createMenu('👀輸出章節(Export Chapter)');
    for (var i = 0; i < para.length; i++) {
      if (levels[i] == 2 && para[i].getText() != "" && para[i].getText() != null) {
        var dynamicMenu = para[i].getText();
        this["function" + i] = dynamicItem(i);
        subMenu.addItem(dynamicMenu, "function" + i);
      }
    }
    if (dynamicMenu==null) {
      console.error("還未設置章節，詳情請觀看示範影片\nhttps://www.youtube.com/watch?v=9SKpUYWjXJY");
      fatal_error = true;
    }
    if (fatal_error===true) {
    ...
    menu.addSubMenu(subMenu).addItem('📉詞彙頻率(Words frequency)', 'frequency').addItem('🎥示範影片(Demo Video)', 'demoVideo').addSeparator().addItem('💽下載TXT(Save as txt)(暫未開放)', 'saveAsTxt').addItem('⬜空白兩格(Spacing)(暫未開放)', 'spacing').addToUi();
    installed = true;
