...
function installFunctions() {
  if (!(userProperties.getProperty('installed')==="true"||fatal_error)) {
    ...
    setLinks();
    setCards();
    let menu = ui.createMenu('Google Docs Writing Engine v0.7.0');
...
function wikiViewer() {
  script = "<script>window.location.href='https://www.wikipedia.org/'</script>";
  displayModeless(script, "👁️檢視器(Viewer)");
}
...
function saveAsTxt(){
  ui.alert("🙅🏻‍♂️This function is not available yet", "這項功能尚未開放，敬請期待！", ui.ButtonSet.OK);
}
